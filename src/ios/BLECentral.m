...
- (void)doDiscoverServices:(CBPeripheral *)peripheral {
  NSLog(@"BLECentral::doDiscoverServices");
...
- (CBService *)doDiscoverServices:(CBPeripheral *)peripheral UUID:(NSString *)UUID {
  ...
  for (CBService *service in peripheral.services) {      
    if ([service.UUID isEqual:[CBUUID UUIDWithString:UUID]]) return service;
  }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
  ...
  for (CBService *service in peripheral.services) {
    if ([_serviceUUID isEqualToString:[self CBUUIDToString:service.UUID]]){
      ...
      NSLog(@"service not found -- %@". [self CBUUIDToString:service.UUID]);
    }
  }
}
...
-(CBCharacteristic *) doDiscoverCharacteristic:(CBService*)service UUID:(NSString *)UUID {
  ...
  for (CBCharacteristic *characteristic in service.characteristics) {      
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:UUID]]) return characteristic;
  }
...
-(void) doReadValueForCharacteristic:(NSString *)serviceUUID characteristicUUID:(NSString *)characteristicUUID {
  NSLog(@"BLECentral::doReadValueForCharacteristic");
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
  ...
  NSLog(@"BLECentral::didUpdateValueForCharacteristic -- %@", stringFromData);
...
-(void) doWriteValueForCharacteristic:(NSString *)serviceUUID characteristicUUID:(NSString *)characteristicUUID data:(NSData *)data {
  NSLog(@"BLECentral::doWriteValueForCharacteristic -- %@", data);
  if(!self.activePeripheral) return;
  ...
  [activePeripheral writeValue:data forCharacteristic:characteristic type:CBCharacteristicWriteWithResponse];
}
...
-(void) doWriteValueForCharacteristic:(CBUUID *)serviceUUID characteristicUUID:(CBUUID *)characteristicUUID peripheral:(CBPeripheral *)peripheral data:(NSData *)data {
  ...
  CBService *service = [self doDiscoverServices:peripheral UUID:[self CBUUIDToString:serviceUUID]];
  if (!service) return;
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
  ...
  if (error) {
    ...
    [alert show];
    NSLog(@"BLECentral::didWriteValueForCharacteristic -- error: %@",[error localizedDescription]);
    return;
  }
  
  NSLog(@"BLECentral::didWriteValueForCharacteristic");
...
//  if (![characteristic.UUID isEqual:[CBUUID UUIDWithString:TRANSFER_CHARACTERISTIC_UUID]]) return;
